...
const ACTIVE_LIVE_STATUSES = new Set(["acquiring", "running", "paused", "live"]);
...
function FooterInner() {
  ...
  const { hydrated: liveHydrated, liveActivity, clearLiveActivity } = useLiveActivity();
  ...
  const liveStatus = String(liveActivity?.status || "").toLowerCase();
  const liveSessionKey = String(liveActivity?.sessionKey || "").trim();
  const liveUpdatedAt = Number(liveActivity?.updatedAt || 0);
  const isFreshLiveState = liveUpdatedAt > 0 && Date.now() - liveUpdatedAt < LIVE_STALE_MS;
  const hasLiveRouteShape = /^\/train\/session\/.+\/live$/.test(liveRouteForMatch);
  const hasValidLiveState =
    !!liveActivity?.isActive &&
    !!liveSessionKey &&
    hasLiveRouteShape &&
    ACTIVE_LIVE_STATUSES.has(liveStatus) &&
    (liveStatus !== "paused" || isFreshLiveState);
  ...
  useEffect(() => {
    ...
    clearLiveActivity();
  }, [clearLiveActivity, hasValidLiveState, liveActivity?.isActive, liveHydrated]);
